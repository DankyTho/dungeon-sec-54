...
class AFK(commands.Cog):
	...
	async def afk(self, ctx, *, args = None):
		...
		if args == None:
			...
			afkmsg = args
		await ctx.send(embed = discord.Embed(description = f"User is Been Afk** : <@{ctx.author.id}>\n**AFK MESSAGE** : {afkmsg}", color = 0x2f3136, timestamp = datetime.datetime.now()))
		
		afk = {"id": ctx.author.id, "msg": afkmsg}
		with open("afk.json", "r") as afklist:
		...
		if afk["id"] in l2:
			...
			l1.append(afk)
		with open("afk.json", "w") as afklist:
	...
	async def on_message(self, message):
		...
		for user in message.mentions:
			if user == message.author:
				...
				if user.id in keys:
					await message.channel.send(embed = discord.Embed(description = f"{user.mention} is currently AFK: {msgs[keys.index(user.id)]}", color = 0x2f3136, timestamp = datetime.datetime.now()))
